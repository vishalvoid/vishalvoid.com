...
const SidebarWrapper: React.FC<{ children: React.ReactNode }> = ({
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="w-full mx-auto max-w-[1100px] px-4 flex">
        {/* Desktop sidebar (sticky, shifts content right) */}
        <aside
          className={`hidden md:flex flex-col justify-between w-full gap-4 md:h-screen transition-all duration-300 border-r border-gray-200 dark:border-slate-700 bg-white dark:bg-stone-950 ${
            enabled
              ? "p-4 md:overflow-hidden md:w-72 md:opacity-100 md:pointer-events-auto"
              : "p-0 md:overflow-hidden md:w-0 md:opacity-0 md:pointer-events-none"
          }`}
          aria-hidden={!enabled}
        >
          <div>
        ...
        <div className="flex-1 min-h-0 overflow-auto transition-all duration-300">
          {children}
        </div>
      </div>
    </div>
